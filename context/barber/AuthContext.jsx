...
export const AuthProvider = ({ children }) => {
  ...
  const fetch_user_email_from_storage = async () => {
    try {
      const savedUserEmail = (await AsyncStorage.getItem("barberEmail")) || "";
      const savedUserSalonId =
        (await AsyncStorage.getItem("barberSalonId")) || 0;
  ...
  const fetch_loggedin_barber_data = async () => {
    try {
      const { data } = await api.post("/web-app/barber/barberloggedin", {
        email: userEmail,
      });
      setUser(data?.user?.[0]);
    } catch (error) {

...
const SignIn = () => {
  ...
  useEffect(() => {
    const fetch_admin_remember_me_email = async () => {
      const barber_remember_me_email = await SecureStore.getItemAsync(
        "barber_remember_me_email",
      );

      if (barber_remember_me_email) {
        setEmail(barber_remember_me_email);
        setRememberMe(true);
      }
    };
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      const { data } = await api.post("/barber/login", payload);
  
      await SecureStore.setItemAsync("barberRefreshToken", data.accessToken);
      await SecureStore.setItemAsync("barberEmail", data?.foundBarber?.email);

      if (rememberMe) {
        await SecureStore.setItemAsync("barber_remember_me_email", email);
      } else {
        await SecureStore.setItemAsync("barber_remember_me_email", "");
      }

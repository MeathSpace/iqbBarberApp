...
import { FlatList, Platform, StyleSheet, Text, TextInput, View, TouchableOpacity, ScrollView, Modal, Pressable } from "react-native";
...
const AppointmentsDashboard = () => {
  ...
  const renderAppointmentCard = ({ item }) => (
    ...
    >
      ...
      <View style={styles.cardFooterRow}>
        <View style={[styles.phoneActionButtonRow, { backgroundColor: "rgba(255, 149, 0, 0.04)", borderRadius: scale(8) }]}>
  ...
  return (
    ...
    >
      <BarberHeader title="Appointment List" subTitle="Live appointment schedule manager" showBack={false} />
      ...
      {/* Premium Luxury Bottom Action Sheet Modal */}
      <Modal
        animationType="slide"
        transparent={true}
      ...
      >
        ...
        >
          <Pressable 
            style={[styles.modalContentPane, { backgroundColor: darkTheme.colors.card, borderColor: darkTheme.colors.border }]}
            pointerEvents="box-none"
          >
            {/* Context Drag Indicator Handle */}
              ...
              <View style={styles.modalBodyWrapper}>
                ...
                <View style={styles.modalHeaderRow}>
                  <View style={styles.modalHeaderTitleBlock}>
                    <Text style={[darkTheme.typography.headerTitle, styles.modalClientTitle]}>
                      {selectedAppointment.clientName}
                    </Text>
                    <Text style={[darkTheme.typography.bodyMuted, styles.modalSubtitle]}>
                      {selectedAppointment.email} • {selectedAppointment.phone}
                    </Text>
                  </View>
                  <TouchableOpacity 
                    style={styles.modalCloseIconCircle} 
                    onPress={() => setModalVisible(false)}
                  >
                ...
                {/* Structured Overview Meta Matrix Grid */}
                <View style={[styles.metaGridMatrix, { backgroundColor: "rgba(255,255,255,0.02)", borderColor: "rgba(255,255,255,0.04)" }]}>
                  <View style={styles.matrixRow}>
                    <View style={styles.matrixColumn}>
                      <Text style={[darkTheme.typography.bodyMuted, styles.matrixLabel]}>SERVICE</Text>
                      <Text style={[darkTheme.typography.bodyMain, styles.matrixValue]}>{selectedAppointment.service}</Text>
                    </View>
                    <View style={styles.matrixColumn}>
                      <Text style={[darkTheme.typography.bodyMuted, styles.matrixLabel]}>ASSIGNED STYLIST</Text>
                      <Text style={[darkTheme.typography.bodyMain, styles.matrixValue]}>{selectedAppointment.stylist}</Text>
                    </View>
                  </View>

                  <View style={[styles.fineDividerLine, { marginVertical: verticalScale(8), backgroundColor: "rgba(255,255,255,0.03)" }]} />

                  <View style={styles.matrixRow}>
                    <View style={styles.matrixColumn}>
                      <Text style={[darkTheme.typography.bodyMuted, styles.matrixLabel]}>SCHEDULED TIME</Text>
                      <View style={{ flexDirection: "row", alignItems: "center", marginTop: verticalScale(2) }}>
                        <Ionicons name="time-outline" size={scale(12)} color={darkTheme.colors.accent} style={{ marginRight: scale(4) }} />
                        <Text style={[darkTheme.typography.bodyMain, styles.matrixValue, { marginTop: 0 }]}>{selectedAppointment.timeSlot}</Text>
                      </View>
                    </View>
                    <View style={styles.matrixColumn}>
                      <Text style={[darkTheme.typography.bodyMuted, styles.matrixLabel]}>AMOUNT DUE</Text>
                      <Text style={[darkTheme.typography.bodyMain, styles.matrixValue, { color: darkTheme.colors.accent, fontWeight: "700" }]}>{selectedAppointment.price}</Text>
                    </View>
                  </View>
                </View>

                {/* Micro Action Button Infrastructure Group */}
                <View style={styles.modalActionGroupRow}>
                  <TouchableOpacity
                    activeOpacity={0.8}
                    onPress={handleCancelAction}
                    style={[styles.actionBtnBase, styles.cancelActionButton]}
                  >
                    <MaterialCommunityIcons name="calendar-remove" size={scale(15)} color="#FF3B30" style={{ marginRight: scale(6) }} />
                  ...
                  <TouchableOpacity
                    ...
                    style={[styles.actionBtnBase, styles.serveActionButton, { backgroundColor: darkTheme.colors.accent }]}
                  >
                    <Ionicons name="checkmark-circle-outline" size={scale(15)} color="#FFFFFF" style={{ marginRight: scale(6) }} />
                    <Text style={styles.serveBtnText}>Serve Client</Text>
                  </TouchableOpacity>
                </View>
              </View>
            )}
...
const styles = StyleSheet.create({
  ...
  // Bottom Modal Component Matrix Layout Style Definitions
  modalOverlay: {
    flex: 1,

...
const EditProfileScreen = () => {
  ...
  const [countryCca2, setCountryCca2] = useState(
    authenticatedUser?.cca2 || "GB",
  );

  const [mobNumber, setMobNumber] = useState(
    authenticatedUser?.mobileNumber
      ? String(authenticatedUser.mobileNumber) // Pass ONLY the national digits (e.g. "1234567890")
      : "",
  );

  // Add this effect hook to handle initial phone validation safely
  useEffect(() => {
    if (mobNumber && phoneInputRef.current) {
      const valid = phoneInputRef.current.isValidNumber(mobNumber);
  ...
  const stripeConnectHandler = async () => {
    try {
      ...
      const { data } = await api.post("/onboard-vendor-account", onboardData);
        ...
        >
          ...
          <View style={styles.formContainerVerticalDeck}>
            ...
            <View style={styles.inputLayoutContainerGroup}>
              ...
              >
                {/* --- UPDATE THE PHONEINPUT COMPONENT PROPERTIES --- */}
                <PhoneInput
                  ref={phoneInputRef}
                  value={mobNumber}
                  ...
                  onChangeCountry={(country) => {
                    ...
                    const newCca2 = country?.cca2;

                    if (newCode && newCode !== countryCode)
                      setCountryCode(newCode);
                    if (newCca2 && newCca2 !== countryCca2)
                      setCountryCca2(newCca2);
                  }}
                  withDarkTheme={true}
                  containerStyle={[
...
//     // callingCode: ["355"],
//     cca2: "DZ", // This is your ISO 3166-1 alpha-2 country code
//     // currency: ["ALL"],
//     // flag: "flag-al",
//     // name: "Albania",
//     // region: "Europe",
...
//         ref={phoneInputRef}
//         value={mobNumber}
//         // 1. Pass the country code string ("AL") from your object here
//         defaultCode={countryData.cca2}
//         onChangeText={(text) => {
//           setMobNumber(text); // Keep state updated when user types
//         }}
//         onChangeCountry={(country) => {
//           // 2. Optional: If the user changes the country manually,
//           // you can update your countryData state like this:
//           console.log(country)

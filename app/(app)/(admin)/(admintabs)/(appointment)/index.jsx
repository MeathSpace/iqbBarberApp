...
const AppointmentsDashboard = () => {
  ...
  const displayAppointments = useMemo(() => {
    ...
    // Filters down the database response data based on selected day and target string query match
  ...
  const renderAppointmentCard = (item) => {
    ...
    const handlePhoneCall = async (countryCode, mobileNumber) => {
      const phoneNumber = `tel:+${countryCode}${mobileNumber}`;
      try {
        // Direct execution handles real devices immediately; catches simulator errors elegantly
        await Linking.openURL(phoneNumber);
      } catch (error) {
        Alert.alert(
          "Call Failed",
...
const styles = StyleSheet.create({
  ...
  // --- RESTORED NOTES SUBSYSTEM STRUCTURE ---
  notesOuterWrapper: {

...
const editSalonInfoStep = () => {
  const router = useRouter();
  const { editSalonInfo, setEditSalonInfo } = useAdminGlobal();
  ...
  const phoneNumberValue = editSalonInfo?.phoneNumber
    ? String(editSalonInfo.phoneNumber)
    : "";

  useEffect(() => {
    if (phoneNumberValue && phoneInputRef.current) {
      const valid = phoneInputRef.current.isValidNumber(phoneNumberValue);
  ...
  }, [phoneNumberValue, editSalonInfo?.countryCca2]);

  const handleInputChange = (key, value) => {
    setEditSalonInfo((prev) => ({ ...prev, [key]: value }));
  ...
  const validateForm = () => {
    ...
    if (!editSalonInfo?.salonName?.trim()) {
      newErrors.salonName = "Salon name is required.";
      isValid = false;
    }
    if (
      !editSalonInfo?.salonEmail?.trim() ||
      !EMAIL_REGEX.test(editSalonInfo.salonEmail.trim())
    ) {
      newErrors.salonEmail = "Please enter a valid email address.";
      isValid = false;
    }
    if (!editSalonInfo?.description?.trim()) {
      newErrors.description = "Salon description is required.";
      isValid = false;
    }

    // Direct check against ref to avoid state-sync delays on submit
    const currentPhone = phoneNumberValue.trim();
    const phoneValid = phoneInputRef.current
      ? phoneInputRef.current.isValidNumber(currentPhone)
      : isPhoneValid;

    if (!currentPhone || !phoneValid) {
          ...
          <View style={styles.formContainer}>
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                placeholderTextColor={darkTheme.colors.textMuted}
                value={editSalonInfo?.salonEmail || ""}
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                placeholderTextColor={darkTheme.colors.textMuted}
                value={editSalonInfo?.description || ""}
            ...
            <View style={styles.inputGroup}>
              ...
              >
                <PhoneInput
                  ...
                  defaultCode={editSalonInfo?.countryCca2 || "US"}
                  withDarkTheme={true}
                  onChangeText={(text) =>
                    handleInputChange("phoneNumber", text)
                  }
                  onChangeCountry={(country) => {
                    setEditSalonInfo((prev) => ({
                      ...prev,
                      countryCode:
...
export default editSalonInfoStep;

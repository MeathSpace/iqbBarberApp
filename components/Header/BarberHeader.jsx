...
//         iconFamily: "Feather",
//         iconName: "archive",
//         route: "/(barbertabs)/(appointment)/appointmentHistory",
//       },
//     ],
//   }
// ];

// const BarberHeader = ({ title, subTitle, showBack = false }) => {
  ...
  },
  {
    ...
    children: [
      ...
      },
      {
        ...
        iconFamily: "Ionicons",
        iconName: "time-outline",
        route: "/(barbertabs)/(queue)/queueHistory",
      },
    ],
  },
  {
...
const BarberHeader = ({ title, subTitle, showBack = false }) => {
  ...
  const isInTabsLayout = segments.includes("(barbertabs)");
  ...
  const logoutPressed = async () => {
    try {
      ...
      closeMenu();
      router.replace("/(auth)/(barberauth)/signin");
    } catch (error) {
      console.error("Error during logout sequence:", error);
    }
  };
  ...
  return (
    <View style={styles.headerRow}>
        ...
        <View style={styles.fullscreenContainer}>
          ...
          >
            ...
            >
              ...
              {/* PREMIUM DEDICATED TOGGLE MODULE CARD - MOVED BELOW MENU */}
              <View style={styles.statusControlCardContainer}>
                {/* Clock In / Out Toggle Item */}
                <TouchableOpacity
                  activeOpacity={0.8}
                  onPress={() => setIsClockedIn(!isClockedIn)}
                  style={styles.toggleRowItem}
                >
                  <View style={styles.toggleLeftBlock}>
                    <View style={styles.toggleIconFrame}>
                      <Feather
                        name="clock"
                        size={scale(13)}
                        color={
                          isClockedIn ? "#4CD964" : darkTheme.colors.textMuted
                        }
                      />
                    </View>
                    <Text
                ...
                {/* Micro separation divider line inside the status module */}
                <View style={styles.statusCardInnerDivider} />

                {/* Online / Offline Toggle Item */}
                <TouchableOpacity
                  activeOpacity={0.8}
                  ...
                  <View style={styles.toggleLeftBlock}>
                    <View style={styles.toggleIconFrame}>
                      <Ionicons
                        ...
                        size={scale(13)}
                        color={
                          isOnline ? "#4CD964" : darkTheme.colors.textMuted
                        }
                      />
                    </View>
                    <Text
      ...
      {/* Premium Bottom-Sheet Modal for Dynamic Salon Profile Modifications */}
      <Modal
        transparent={true}
        visible={salonModalVisible}
        onRequestClose={closeSalonModal}
...
const styles = StyleSheet.create({
  ...
  // Premium Isolated Toggle Module Styling (Placed Below Links Block)
  statusControlCardContainer: {
